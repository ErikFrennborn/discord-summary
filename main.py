...
def number_of_words(data):
    ...
    for message in data:
        author = get_author(message)
        number_of_words = len(message["content"].strip().split())
...
def print_sort_tuple(data):
    ...
    data_list=list(map(lambda x:(x[0],(x[1],round(100*x[1]/total))),data_list))
...
def print_data():
    ...
    print_sort_tuple(number_of_words(messages))
